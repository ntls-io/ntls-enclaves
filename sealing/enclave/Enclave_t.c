...
typedef struct ms_seal_user_data_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_user_id;
	size_t ms_user_id_length;
	uint8_t* ms_user_data;
	size_t ms_sealed_user_data_size;
	uint8_t* ms_sealed_user_sealing_key;
} ms_seal_user_data_t;
...
static sgx_status_t SGX_CDECL sgx_seal_user_data(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_seal_user_data_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_seal_user_data_t* ms = SGX_CAST(ms_seal_user_data_t*, pms);
	ms_seal_user_data_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_seal_user_data_t), ms, sizeof(ms_seal_user_data_t))) {
	...
	uint8_t* _tmp_user_data = __in_ms.ms_user_data;
	size_t _tmp_sealed_user_data_size = __in_ms.ms_sealed_user_data_size;
	size_t _len_user_data = _tmp_sealed_user_data_size;
	uint8_t* _in_user_data = NULL;
	uint8_t* _tmp_sealed_user_sealing_key = __in_ms.ms_sealed_user_sealing_key;
	size_t _len_sealed_user_sealing_key = 48;
	uint8_t* _in_sealed_user_sealing_key = NULL;
	sgx_status_t _in_retval;

	CHECK_UNIQUE_POINTER(_tmp_user_id, _len_user_id);
	CHECK_UNIQUE_POINTER(_tmp_user_data, _len_user_data);
	CHECK_UNIQUE_POINTER(_tmp_sealed_user_sealing_key, _len_sealed_user_sealing_key);
	...
	if (_tmp_user_data != NULL && _len_user_data != 0) {
		if ( _len_user_data % sizeof(*_tmp_user_data) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_user_data = (uint8_t*)malloc(_len_user_data);
		if (_in_user_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_user_data, _len_user_data, _tmp_user_data, _len_user_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_sealed_user_sealing_key != NULL && _len_sealed_user_sealing_key != 0) {
		if ( _len_sealed_user_sealing_key % sizeof(*_tmp_sealed_user_sealing_key) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_user_sealing_key = (uint8_t*)malloc(_len_sealed_user_sealing_key)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_user_sealing_key, 0, _len_sealed_user_sealing_key);
	}
	_in_retval = seal_user_data((const uint8_t*)_in_user_id, _tmp_user_id_length, _in_user_data, _tmp_sealed_user_data_size, _in_sealed_user_sealing_key);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_user_data) {
		if (memcpy_verw_s(_tmp_user_data, _len_user_data, _in_user_data, _len_user_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_sealed_user_sealing_key) {
		if (memcpy_verw_s(_tmp_sealed_user_sealing_key, _len_sealed_user_sealing_key, _in_sealed_user_sealing_key, _len_sealed_user_sealing_key)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	...
	if (_in_user_data) free(_in_user_data);
	if (_in_sealed_user_sealing_key) free(_in_sealed_user_sealing_key);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_seal_user_data, 0, 0},
