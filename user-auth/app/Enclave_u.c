...
sgx_status_t hash_password(sgx_enclave_id_t eid, sgx_status_t* retval, const uint8_t* password, size_t password_length, const uint8_t* salt, const uint8_t* user_id, size_t user_id_length, uint8_t* hash_string)
{
	sgx_status_t status;
	ms_hash_password_t ms;
	ms.ms_password = password;
	ms.ms_password_length = password_length;
	ms.ms_salt = salt;
	ms.ms_user_id = user_id;
	ms.ms_user_id_length = user_id_length;
	ms.ms_hash_string = hash_string;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t verify_password(sgx_enclave_id_t eid, sgx_status_t* retval, const uint8_t* password, size_t password_length, const uint8_t* user_id, size_t user_id_length, const uint8_t* stored_hash_string, uint32_t* password_verified)
{
	sgx_status_t status;
	ms_verify_password_t ms;
	ms.ms_password = password;
	ms.ms_password_length = password_length;
	ms.ms_user_id = user_id;
	ms.ms_user_id_length = user_id_length;
	ms.ms_stored_hash_string = stored_hash_string;
	ms.ms_password_verified = password_verified;
