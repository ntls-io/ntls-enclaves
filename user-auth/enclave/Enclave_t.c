...
typedef struct ms_hash_password_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_password;
	size_t ms_password_length;
	const uint8_t* ms_salt;
	const uint8_t* ms_user_id;
	size_t ms_user_id_length;
	uint8_t* ms_hash_string;
} ms_hash_password_t;

typedef struct ms_verify_password_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_password;
	size_t ms_password_length;
	const uint8_t* ms_user_id;
	size_t ms_user_id_length;
	const uint8_t* ms_stored_hash_string;
	uint32_t* ms_password_verified;
...
static sgx_status_t SGX_CDECL sgx_hash_password(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_hash_password_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_hash_password_t* ms = SGX_CAST(ms_hash_password_t*, pms);
	ms_hash_password_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_hash_password_t), ms, sizeof(ms_hash_password_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_password = __in_ms.ms_password;
	size_t _tmp_password_length = __in_ms.ms_password_length;
	size_t _len_password = _tmp_password_length;
	uint8_t* _in_password = NULL;
	const uint8_t* _tmp_salt = __in_ms.ms_salt;
	size_t _len_salt = 22;
	uint8_t* _in_salt = NULL;
	const uint8_t* _tmp_user_id = __in_ms.ms_user_id;
	size_t _tmp_user_id_length = __in_ms.ms_user_id_length;
	size_t _len_user_id = _tmp_user_id_length;
	uint8_t* _in_user_id = NULL;
	uint8_t* _tmp_hash_string = __in_ms.ms_hash_string;
	size_t _len_hash_string = 97;
	uint8_t* _in_hash_string = NULL;
	...
	CHECK_UNIQUE_POINTER(_tmp_hash_string, _len_hash_string);
	...
	if (_tmp_salt != NULL && _len_salt != 0) {
		...
		_in_salt = (uint8_t*)malloc(_len_salt);
		if (_in_salt == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_salt, _len_salt, _tmp_salt, _len_salt)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	if (_tmp_hash_string != NULL && _len_hash_string != 0) {
		if ( _len_hash_string % sizeof(*_tmp_hash_string) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_hash_string = (uint8_t*)malloc(_len_hash_string)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_hash_string, 0, _len_hash_string);
	}
	_in_retval = hash_password((const uint8_t*)_in_password, _tmp_password_length, (const uint8_t*)_in_salt, (const uint8_t*)_in_user_id, _tmp_user_id_length, _in_hash_string);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_hash_string) {
		if (memcpy_verw_s(_tmp_hash_string, _len_hash_string, _in_hash_string, _len_hash_string)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	...
	if (_in_hash_string) free(_in_hash_string);
	return status;
}

static sgx_status_t SGX_CDECL sgx_verify_password(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_verify_password_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_verify_password_t* ms = SGX_CAST(ms_verify_password_t*, pms);
	ms_verify_password_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_verify_password_t), ms, sizeof(ms_verify_password_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_password = __in_ms.ms_password;
	...
	size_t _len_user_id = _tmp_user_id_length;
	uint8_t* _in_user_id = NULL;
	const uint8_t* _tmp_stored_hash_string = __in_ms.ms_stored_hash_string;
	size_t _len_stored_hash_string = 97;
	uint8_t* _in_stored_hash_string = NULL;
	uint32_t* _tmp_password_verified = __in_ms.ms_password_verified;
	size_t _len_password_verified = sizeof(uint32_t);
	uint32_t* _in_password_verified = NULL;
	sgx_status_t _in_retval;

	CHECK_UNIQUE_POINTER(_tmp_password, _len_password);
	CHECK_UNIQUE_POINTER(_tmp_user_id, _len_user_id);
	CHECK_UNIQUE_POINTER(_tmp_stored_hash_string, _len_stored_hash_string);
	CHECK_UNIQUE_POINTER(_tmp_password_verified, _len_password_verified);
	...
	if (_tmp_stored_hash_string != NULL && _len_stored_hash_string != 0) {
		if ( _len_stored_hash_string % sizeof(*_tmp_stored_hash_string) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_stored_hash_string = (uint8_t*)malloc(_len_stored_hash_string);
		if (_in_stored_hash_string == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_stored_hash_string, _len_stored_hash_string, _tmp_stored_hash_string, _len_stored_hash_string)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_password_verified != NULL && _len_password_verified != 0) {
		if ( _len_password_verified % sizeof(*_tmp_password_verified) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_password_verified = (uint32_t*)malloc(_len_password_verified)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_password_verified, 0, _len_password_verified);
	}
	_in_retval = verify_password((const uint8_t*)_in_password, _tmp_password_length, (const uint8_t*)_in_user_id, _tmp_user_id_length, (const uint8_t*)_in_stored_hash_string, _in_password_verified);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_password_verified) {
		if (memcpy_verw_s(_tmp_password_verified, _len_password_verified, _in_password_verified, _len_password_verified)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_password) free(_in_password);
	if (_in_user_id) free(_in_user_id);
	if (_in_stored_hash_string) free(_in_stored_hash_string);
	if (_in_password_verified) free(_in_password_verified);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_hash_password, 0, 0},
		{(void*)(uintptr_t)sgx_verify_password, 0, 0},
