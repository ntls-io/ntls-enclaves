...
typedef struct ms_row_counter_t {
...
typedef struct ms_dataset_hashing_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_some_string;
	size_t ms_len;
	uint8_t* ms_hash;
} ms_dataset_hashing_t;

typedef struct ms_dataset_append_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_original_data;
	size_t ms_original_data_len;
...
static sgx_status_t SGX_CDECL sgx_row_counter(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_row_counter_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_row_counter_t* ms = SGX_CAST(ms_row_counter_t*, pms);
	ms_row_counter_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_row_counter_t), ms, sizeof(ms_row_counter_t))) {
	...
	uint64_t* _tmp_count = __in_ms.ms_count;
	size_t _len_count = sizeof(uint64_t);
	uint64_t* _in_count = NULL;
	...
	if (_tmp_count != NULL && _len_count != 0) {
		...
		if ((_in_count = (uint64_t*)malloc(_len_count)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	_in_retval = row_counter((const uint8_t*)_in_some_string, _tmp_len, _in_count);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_count) {
		if (memcpy_verw_s(_tmp_count, _len_count, _in_count, _len_count)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_dataset_hashing(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_dataset_hashing_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_dataset_hashing_t* ms = SGX_CAST(ms_dataset_hashing_t*, pms);
	ms_dataset_hashing_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_dataset_hashing_t), ms, sizeof(ms_dataset_hashing_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_some_string = __in_ms.ms_some_string;
	...
	size_t _len_some_string = _tmp_len;
	uint8_t* _in_some_string = NULL;
	uint8_t* _tmp_hash = __in_ms.ms_hash;
	size_t _len_hash = 64;
	...
	_in_retval = dataset_hashing((const uint8_t*)_in_some_string, _tmp_len, _in_hash);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
static sgx_status_t SGX_CDECL sgx_dataset_append(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_dataset_append_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_dataset_append_t* ms = SGX_CAST(ms_dataset_append_t*, pms);
	ms_dataset_append_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_dataset_append_t), ms, sizeof(ms_dataset_append_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_original_data = __in_ms.ms_original_data;
	size_t _tmp_original_data_len = __in_ms.ms_original_data_len;
	size_t _len_original_data = _tmp_original_data_len;
	uint8_t* _in_original_data = NULL;
	const uint8_t* _tmp_new_data = __in_ms.ms_new_data;
	size_t _tmp_new_data_len = __in_ms.ms_new_data_len;
	...
	if (_tmp_original_data != NULL && _len_original_data != 0) {
		if ( _len_original_data % sizeof(*_tmp_original_data) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_original_data = (uint8_t*)malloc(_len_original_data);
		if (_in_original_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_original_data, _len_original_data, _tmp_original_data, _len_original_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_new_data != NULL && _len_new_data != 0) {
		if ( _len_new_data % sizeof(*_tmp_new_data) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_new_data = (uint8_t*)malloc(_len_new_data);
		if (_in_new_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_new_data, _len_new_data, _tmp_new_data, _len_new_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
...
SGX_EXTERNC const struct {
	...
	5,
	{
		{(void*)(uintptr_t)sgx_row_counter, 0, 0},
		{(void*)(uintptr_t)sgx_dataset_hashing, 0, 0},
		{(void*)(uintptr_t)sgx_dataset_append, 0, 0},
